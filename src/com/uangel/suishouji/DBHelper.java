...
//		db.execSQL("insert into "+TABLE_NAME+"(name,pwd,position,resource) values(?,?,?,?)", new String[]{"hvftu","hvftu","团支书","B"});
//		db.execSQL("insert into "+TABLE_NAME+"(name,pwd,position,resource) values(?,?,?,?)", new String[]{"弡上","aibohtbo","学生","C"});
//		db.execSQL("insert into "+TABLE_NAME+"(name,pwd,position,resource) values(?,?,?,?)", new String[]{"杏囜","mjtj","学生","D"});
	...
	public void insert(String name,String pwd,String s1,String PSMail,String sex){
		//System.out.println("被调用1");
		SQLiteDatabase db=this.getWritableDatabase();
		//System.out.println("被调用1。1");
		db.execSQL("insert into "+TABLE_NAME+"(name,pwd,question,mail,sex) values(?,?,?,?,?)", new String[]{name,pwd,s1,PSMail,sex});
